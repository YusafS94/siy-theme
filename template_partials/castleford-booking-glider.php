<div class="glide">
    <div class="glide__track" data-glide-el="track">
      <ul class="glide__slides">

        <li class="glide__slide">
            <!-- Card 1 -->
            <img class="about-img border border-dark booking-card-img" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/2.jpg" alt="">
            <div class="">
                <div class="d-flex flex-column align-items-center text-center">
                    <p>All</p>
                    <h4>View all club packages</h4>
                    <p>View full range of packages available at the <strong>(Address TBC)</strong>. Open to customers and club members.</p>
                    <p>All equipment provided for free!</p>
                    <a target="blank" href="https://siybooking.as.me/?appointmentType=category:Orange%20Box" class="btn btn-warning rounded-pill py-2 px-4">Book</a>
                </div>
            </div>
        </li>

        <li class="glide__slide">
            <!-- Card 2 -->
            <img class="about-img border border-dark" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/2.jpg" alt="">
            <div class="">
                <div class="d-flex flex-column align-items-center text-center">
                    <p>45 minutes @ £30.00</p>
                    <h4>1-1 private session</h4>
                    <p>45 minute private lesson with expert coaches and access to private skatepark open to 1-1 customers <strong>(Address TBC)</strong>.</p>
                    <p>All equipment provided for free!</p>
                    <a target="blank" href="https://siybooking.as.me/PrivateLesson" class="btn btn-warning rounded-pill py-2 px-4">Book</a>
                </div>
            </div>
        </li>

        <li class="glide__slide">
            <!-- Card 3 -->
            <img class="about-img border border-dark" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/2.jpg" alt="">
            <div class="">
                <div class="d-flex flex-column align-items-center text-center">
                    <p>2 hours @ £3.00</p>
                    <h4>2 hour public access</h4>
                    <p>2 hour public access to the skate park <strong>(Address TBC)</strong>.</p>
                    <p>All equipment provided for free!</p>
                    <a target="blank" href="https://siybooking.as.me/OrangeBox" class="btn btn-warning rounded-pill py-2 px-4">Book</a>
                </div>
            </div>
        </li>
      </ul>
    </div>
    <div class="glide__arrows justify-content-end" data-glide-el="controls">
        <button class="glide__arrow glide__arrow--left bg-warning" id="custom-left-arrow" data-glide-dir="<"><i class="fa fa-arrow-left"></i></button>
        <button class="glide__arrow glide__arrow--right bg-warning" id="custom-right-arrow" data-glide-dir=">"><i class="fa fa-arrow-right"></i></button>
    </div>
  <div class="glide__bullets bg-warning" data-glide-el="controls[nav]">
    <button class="glide__bullet" data-glide-dir="=0"></button>
    <button class="glide__bullet" data-glide-dir="=1"></button>
    <button class="glide__bullet" data-glide-dir="=2"></button>
  </div>
</div>